...
from utils.volume_op_Tcomp import volume_sampling_ndc, volume_spec_rendering_klen
...
def parse_args():
    ...
    parser.add_argument('--bf_num', type=int, default=5, help='number of freqs for basis function')
...
def train_one_epoch(scene_train, optimizer_nerf, optimizer_focal, optimizer_pose, optimizer_basis, 
    ...
    # tonemapping = lambda sg_x, x, : x   # weight loss
    tonemapping = lambda sg_x, x, : x / torch.sqrt( sg_x.detach() + 1e-3 )    # weight loss
    ...
    # Lap = 1e-2 * bf @ L @ L @ bf.T # ( N_BF, C ) x ( C, C ) x ( C, C ) x ( C, N_BF ) -> ( N_BF, N_BF )
    # Minv = torch.linalg.inv( PHI.T @ PHI + Lap ) @ PHI.T  # ( N_BF, 3 x N ) * ( 3 x N, N_BF ) * ( N_BF, 3 x N ) -> ( N_BF, 3 x N )
    ...
    for i in range(N_img):
        ...
        img = scene_train.imgs[i].to(my_devices)  # (H, W, 1)
        ...
        comp_spec_rendered = render_result['spec']  # (H, W, 2xBF+1])
        ...
        # Orthogonal loss (Factorized Representation Learning)
        # L2_orth = F.mse_loss( bf @ bf.T, torch.eye( 7 ).to( my_devices ) )
        L2_orth = torch.tensor(0).to( device = my_devices )

        # Laplacian smooth
        # spec = torch.matmul( bf.transpose(1, 0)[None, None, ...], comp_spec_rendered[..., None] )  # [1, 1, C, BF] x [H, W, 2xBF+1, 1] -> [H, W, SC, 1]
        # spec = spec.clamp( 0, 1 )
        # L2_spec = torch.mean( torch.matmul( L, spec ) ** 2 )   # [C, C] x [H, W, C, 1] -> [H, W, C]

        L2_raw_loss = F.mse_loss( rgb_rendered[i], img_selected )  # loss for one image
        
        # L2_loss = F.mse_loss( tonemapping(rgb_rendered[i], rgb_rendered[i]), tonemapping(rgb_rendered[i], img_selected) )  # loss for one image
        # L2_loss = L2_raw_loss
        L2_loss = L2_lsm
        ...
        L2_orth_epoch.append(L2_orth.item())
...
def main(args):
    ...
    shutil.copy('./tasks/any_folder_spec_klensPlus_RGB_coding/train_Tcomp.py', experiment_dir)
    shutil.copy('./tasks/any_folder_spec_klensPlus_RGB_coding/spiral_Tcomp.py', experiment_dir)
    shutil.copy('./dataloader/any_folder_spec_klensPlus_RGB_Tcomp.py', experiment_dir)
    shutil.copy('./utils/volume_op_Tcomp.py', experiment_dir)
    ...
    for epoch_i in tqdm(range(args.epoch), desc='epochs'):
        ...
        train_L2_orth_loss = train_epoch_losses['L2_orth']
        ...
        train_orth_psnr = mse2psnr(train_L2_orth_loss)
        ...
        logger.info('{0:6d} ep: Train: L2 raw loss: {1:.5f}, raw_PSNR: {2:.3f}, L2 loss: {3:.5f}, PSNR: {4:.3f}, depth loss: {5:.5f}, PSNR: {6:.3f}, orth loss: {7:.4f}, PSNR: {8:.3f}'
                    .format(epoch_i, train_L2_raw_loss, train_raw_psnr, train_L2_loss, train_psnr, train_L_depth_loss, train_depth_psnr, train_L2_orth_loss, train_orth_psnr))
        tqdm.write('{0:6d} ep: Train: L2 raw loss: {1:.5f}, raw_PSNR: {2:.3f}, L2 loss: {3:.5f}, PSNR: {4:.3f}, depth loss: {5:.5f}, PSNR: {6:.3f}, orth loss: {7:.4f}, PSNR: {8:.3f}'
                    .format(epoch_i, train_L2_raw_loss, train_raw_psnr, train_L2_loss, train_psnr, train_L_depth_loss, train_depth_psnr, train_L2_orth_loss, train_orth_psnr))
