...
from tasks.any_folder_spec_klensPlus_RGB_coding.train_gt import model_render_image
...
def parse_args():
    ...
    parser.add_argument('--bf_num', type=int, default=6, help='number of freqs for basis function')
...
def main(args):
    ...
    spec_gt = scene_train.gt.to(my_devices) # (N, H, W, C)
    ...
    '''Stare'''
    L = laplacian( args.spec_chnls ).to(device=my_devices)
    _sens_nrngnb = rgb_sens.permute( [2, 0, 1] ).reshape( 9 * 3, -1 )  # ( N, C, 3 ) -> ( 3 x N, C )   [r..., g..., b...]
    PHI = _sens_nrngnb @ bf.T # ( 3 x N, C ) * ( C, N_BF ) -> ( 3 x N, N_BF )
    Lap = 1e-4 * bf @ L @ L @ bf.T # ( N_BF, C ) x ( C, C ) x ( C, C ) x ( C, N_BF ) -> ( N_BF, N_BF )
    ...
    if args.stare:
        ...
        for s in range( rgb_sens.shape[0] ):  # [9 views]
            ...
            spec = result['spec'][0] @ bf  # [H, W, BF] x [BF, C] x  -> [H, W, C]
            spec = spec.clamp(0 ,1)

            spec_psnr, spec_ssim, spec_sam, spec_rmse = evaluate_metrics(spec_gt[s], spec)
            avg_spec_psnr.append(spec_psnr)
            avg_spec_ssim.append(spec_ssim)
            avg_spec_sam.append(spec_sam)
            avg_spec_rmse.append(spec_rmse)

            # L2_spec_loss = F.mse_loss( spec, spec_gt[s] )
            # spec_psnr = mse2psnr( L2_spec_loss.cpu().numpy() )
            print( f"PSNR of spectral for view { s + 1 }: { spec_psnr }" )
            avg_spec_psnr.append( spec_psnr )
        ...
        print( f"gt mean PSNR: { np.mean(avg_psnr) }" )
        print( f"gt mean spectral PSNR: { np.mean(avg_spec_psnr) }" )
        print( f"gt mean spectral SSIM: { np.mean(avg_spec_ssim) }" )
        print( f"gt mean spectral SAM: { np.mean(avg_spec_sam) }" )
        print( f"gt mean spectral RMSE: { np.mean(avg_spec_rmse) }" )

        # save metric
        filename = os.path.join(test_dir, 'metrics.txt')
        with open( filename , "w") as f:
            f.write(f"file info : {filename}\n")
            f.write(f"mean spectral PSNR : {np.mean(avg_spec_psnr):.4f} dB\n")
            f.write(f"mean spectral SSIM : {np.mean(avg_spec_ssim):.4f} \n")
            f.write(f"mean spectral SAM  : {np.mean(avg_spec_sam):.4f} rad \n")
            f.write(f"mean spectral RMSE : {np.mean(avg_spec_rmse):.4f}\n")
