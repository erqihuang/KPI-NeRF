...
def volume_spec_rendering_klen(spec_density, t_vals, sigma_noise_std, spec_act_fn, rgb_sens, bf):
    ...
    coeff = spec_act_fn( spec_density[:, :, :, :-1] )  # (H, W, N_sample, C)
    sigma_a = spec_density[:, :, :, -1]  # (H, W, N_sample)
    ...
    coeff_rendered = torch.sum( weight.unsqueeze(3) * coeff, dim=2 )    # (H, W, N_sample, 1) * (H, W, N_sample, N_BF) = (H, W, N_sample, N_BF) -> (H, W, N_BF)
    
    depth_map = torch.sum( weight * t_vals, dim=2 )  # (H, W)
    
    _spec = coeff_rendered @ bf # (H, W, N_BF) x (N_BF, C) -> (H, W, C)
    rgb_rendered = torch.einsum('hwc,nct->nhwt', _spec, rgb_sens )  # (H, W, C) x (N, C, 3) -> (N, H, W, 3)
    rgb_rendered = rgb_rendered.clip(min=0, max=1)
    
    result = {
        'spec': coeff_rendered,  # (H, W, C)
