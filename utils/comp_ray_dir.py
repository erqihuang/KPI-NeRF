import torch
from utils.vis_cam_traj import save_off
...
def comp_ray_dir_cam_fxfy_vis(H, W, fx, fy):
    """Compute ray directions in the camera coordinate, which only depends on intrinsics.
    This could be further transformed to world coordinate later, using camera poses.
    :return: (H, W, 3) torch.float32
    """
    ...
    #HACK: Visualization-ray camera coordinate (RUB)
    points = rays_dir.reshape((-1,3)).cpu().detach().numpy()    # [N_points, 3]
    filename = './visualization/0_camera.off'
    save_off(points, filename)
