...
def get_ndc_finite_rays_fxfy(H, W, fxfy, near, far, rays_o, rays_d):
    # transfer from world coordinate to cameara coordinate
    # near = rays_o[..., 2] + (-near)
    # far = rays_o[..., 2] + (-far)

    # # Shift ray origins to near plane
    # t = ( near - rays_o[..., 2] ) / rays_d[..., 2]
    # t_prime = 1 - ( 1 / ( 1 + t * rays_d[..., 2] / rays_o[..., 2] ) )
    # rays_o = rays_o + t[..., None] * rays_d
    ...
    o0 = -1. / (W / (2. * fxfy[0])) * ox_oz
    o1 = -1. / (H / (2. * fxfy[1])) * oy_oz
    o2 = ( far + near ) / ( far - near ) + ( 2. * far *  near /  ( far - near ) ) / rays_o[..., 2]

    d0 = -1. / (W / (2. * fxfy[0])) * t_prime * (rays_d[..., 0] / rays_d[..., 2] - ox_oz)
    d1 = -1. / (H / (2. * fxfy[1])) * t_prime * (rays_d[..., 1] / rays_d[..., 2] - oy_oz)
    d2 = -( 2. * far *  near /  ( far - near ) ) * t_prime / rays_o[..., 2]
...
def volume_sampling_infinite_finite_ndc(c2w, ray_dir_cam, t_vals, near, far, H, W, focal, perturb_t):
    """
    :param c2w:             (3/4, 4)                camera pose
    :param ray_dir_cam:     (H, W, 3)               ray directions in the camera coordinate
    :param focal:           a float or a (2,)       torch tensor for focal.
    ...
    # sparity sampling
    t_f = - ( far + ray_ori_world[..., 2] ) / ray_dir_world[..., 2]
    t_f_prime = 1 - ( 1 / ( 1 + (-t_f) * ray_dir_world[..., 2] / ray_ori_world[..., 2] ) )
    t_vals_ndc = torch.linspace(0, t_f_prime[0].item(), N_sam, device=t_vals.device)  # (N_sample,) sample position
 
    if isinstance(focal, float):
        ray_ori_world, ray_dir_world = get_ndc_rays(H, W, focal, 1.0, rays_o=ray_ori_world, rays_d=ray_dir_world)  # (H*W, 3)
    else:  # if focal is a tensor contains fxfy
        ray_ori_finite_world, ray_dir_finite_world = get_ndc_finite_rays_fxfy(H, W, focal, 1.999, far, rays_o=ray_ori_world, rays_d=ray_dir_world)  # (H*W, 3)
        ray_ori_world, ray_dir_world = get_ndc_rays_fxfy(H, W, focal, 1.0, rays_o=ray_ori_world, rays_d=ray_dir_world)  # (H*W, 3)
    ray_dir_world = ray_dir_world.reshape(ray_H, ray_W, 3)  # (H, W, 3)
    ray_ori_world = ray_ori_world.reshape(ray_H, ray_W, 3)  # (H, W, 3)

    ray_dir_finite_world = ray_dir_finite_world.reshape(ray_H, ray_W, 3)  # (H, W, 3)
    ray_ori_finite_world = ray_ori_finite_world.reshape(ray_H, ray_W, 3)  # (H, W, 3)
    ...
    if perturb_t:
        ...
        t_noise = t_noise * (far - near) / N_sam
        t_vals_noisy = t_vals.view(1, 1, N_sam) + t_noise  # (H, W, N_sam)
        t_vals_ndc_noisy = t_vals_ndc.view(1, 1, N_sam) + t_noise  # (H, W, N_sam)
    else:
        t_vals_noisy = t_vals.view(1, 1, N_sam).expand(ray_H, ray_W, N_sam)
        t_vals_ndc_noisy = t_vals_ndc.view(1, 1, N_sam).expand(ray_H, ray_W, N_sam)

    # Get sample position in the world (H, W, 1, 3) + (H, W, 1, 3) * (H, W, N_sam, 1) -> (H, W, N_sample, 3)
    # sample_pos = ray_ori_world.unsqueeze(2) + ray_dir_world.unsqueeze(2) * t_vals_noisy.unsqueeze(3)
    sample_pos = ray_ori_finite_world.unsqueeze(2) + ray_dir_finite_world.unsqueeze(2) * t_vals_ndc_noisy.unsqueeze(3)

    # return sample_pos, ray_ori_world, ray_dir_world, t_vals_noisy  # (H, W, N_sample, 3), (3, ), (H, W, 3), (H, W, N_sam)
    return sample_pos, ray_ori_finite_world, ray_dir_finite_world, t_vals_ndc_noisy  # (H, W, N_sample, 3), (3, ), (H, W, 3), (H, W, N_sam)


def volume_sampling_ndc_finite(c2w, ray_dir_cam, t_vals, near, far, H, W, focal, perturb_t):
    """
    :param c2w:             (3/4, 4)                camera pose
    :param ray_dir_cam:     (H, W, 3)               ray directions in the camera coordinate
    :param focal:           a float or a (2,)       torch tensor for focal.
    ...
    # ray sampling
    t_far_prime = 1 - ( 1 / ( 1 + far * ray_dir_world[..., 2] / ray_ori_world[..., 2] ) )
    t_vals_ndc = torch.linspace(0, t_far_prime[0].item(), N_sam, device=t_vals.device)  # (N_sample,) sample position

    ray_ori_finite_world, ray_dir_finite_world = get_ndc_finite_rays_fxfy(H, W, focal, near, far, rays_o=ray_ori_world, rays_d=ray_dir_world)  # (H*W, 3)
    ray_ori_finite_world = ray_ori_finite_world.reshape(ray_H, ray_W, 3)  # (H, W, 3)
    ray_dir_finite_world = ray_dir_finite_world.reshape(ray_H, ray_W, 3)  # (H, W, 3)
...
def volume_rendering(rgb_density, t_vals, sigma_noise_std, rgb_act_fn):
    ...
    rgb = rgb_density[:, :, :, :3]  # (H, W, N_sample, 3)
...
def volume_spec_rendering(spec_density, t_vals, sigma_noise_std, spec_act_fn, gray_sens, rgb_sens):
    """
    :param spec_density:        (H, W, N_sample, 9)        network output
    :param t_vals:              (H, W, N_sample)           compute the distance between each sample points
    :param sigma_noise_std:     A scalar                   add some noise to the density output, this is helpful to reduce
                                                           floating artifacts according to official repo, but they set it to
                                                           zero in their implementation.
    :param rgb_act_fn:          relu()                     apply an active fn to the raw rgb output to get actual rgb
    :param sensitivity:         (9, 1)                     apply sensitivity to the spectral cube to get the spectral image.
    :return:                    (H, W, 9)                  rendered multispectral image
                                (H, W, 1)                  rendered pseudo mono spectral image
                                (H, W, N_sample)           weights at each sample position
    """
    ray_H, ray_W, num_sample = t_vals.shape[0], t_vals.shape[1], t_vals.shape[2]

    spec = spec_density[:, :, :, :-1]  # (H, W, N_sample, 9)
    # spec = spec_act_fn(spec_density[:, :, :, :-1])  # (H, W, N_sample, 9)
    sigma_a = spec_density[:, :, :, -1]  # (H, W, N_sample)
    ...
    # (H, W, N_sample, 1) * (H, W, N_sample, 9) = (H, W, N_sample, 9) -> (H, W, 9)
    spec_rendered = torch.sum(weight.unsqueeze(3) * spec, dim=2)
    
    # (H, W, 9) * (9, 1) = (H, W, 1)
    mono_rendered = ( spec_rendered @ gray_sens ).unsqueeze(-1)
    mono_rendered[ mono_rendered < 0 ], mono_rendered[ mono_rendered > 1 ] = 0, 1

    # (H, W, 9) * (9, 3) = (H, W, 3)
    rgb_rendered = ( spec_rendered @ rgb_sens ).unsqueeze(-1)
    rgb_rendered[ rgb_rendered < 0 ], rgb_rendered[ rgb_rendered > 1 ] = 0, 1

    depth_map = torch.sum( weight * t_vals, dim=2 )  # (H, W)

    result = {
        'spec': spec_rendered,  # (H, W, 9)
        'mono': mono_rendered,  # (H, W, 1)
        'rgb': rgb_rendered,  # (H, W, 3)
        'weight': weight,  # (H, W, N_sample)
        'depth_map': depth_map,  # (H, W)
    }
...
def volume_spec_rendering_klen(spec_density, t_vals, sigma_noise_std, spec_act_fn, rgb_sens, bf):
    """
    :param spec_density:        (H, W, N_sample, 9)        network output
    :param t_vals:              (H, W, N_sample)           compute the distance between each sample points
    :param sigma_noise_std:     A scalar                   add some noise to the density output, this is helpful to reduce
                                                           ...
                                                           zero in their implementation.
    :param rgb_act_fn:          relu()                     apply an active fn to the raw rgb output to get actual rgb
    :param sensitivity:         (N, 9, 3)                  apply sensitivity to the spectral cube to get the spectral image.
    :param bf:                  (bf_C, 27)                 basis function
    :return:                    (H, W, C)                  rendered multispectral image
                                (H, W, 1)                  rendered pseudo mono spectral image
                                (H, W, N_sample)           weights at each sample position
    """
    ray_H, ray_W, num_sample = t_vals.shape
    N = rgb_sens.shape[0]

    spec = spec_act_fn( spec_density[:, :, :, :-1] )  # (H, W, N_sample, C)
    sigma_a = spec_density[:, :, :, -1]  # (H, W, N_sample)
    ...
    # (H, W, N_sample, 1) * (H, W, N_sample, 2*N_BF+1) = (H, W, N_sample, 2*N_BF+1) -> (H, W, 2*N_BF+1)
    spec_rendered = torch.sum( weight.unsqueeze(3) * spec, dim=2 )
    
    # (H, W, 2*N_BF+1) * (2*N_BF+1, 3) = (N, H, W, 3)
    rgb_rendered = []
    
    # rgb_sens_reduced = np.zeros( ( rgb_sens_list.shape[0], bf.shape[0], channel_rgb ), dtype=np.float32 ) # [V, 2*N_BF+1, 3]
    # for i in range( bf.shape[0] ):
    #     rgb_sens_reduced[ :, i, : ] = np.sum( bf[None, i, :, None] * rgb_sens_list_p, axis=1 )   # [V, 2*N_BF+1, 3]

    depth_map = torch.sum( weight * t_vals, dim=2 )  # (H, W)
    attenuate = torch.exp( -1. * torch.sum( alpha * weight * t_vals, dim=2) ).unsqueeze(-1) # (H, W)

    for i in range(N):
        _rgb = spec_rendered @ ( bf @ rgb_sens[i] )  # (H, W, 2*N_BF+1) * (2*N_BF+1, 27) * (27, 3) -> (H, W, 3) 
        # _rgb[ _rgb > 1 ] = 1
        # _rgb = _rgb * attenuate
        rgb_rendered.append( _rgb )
    rgb_rendered = torch.stack( rgb_rendered )
    rgb_rendered = rgb_rendered.clip(min=0, max=1)
